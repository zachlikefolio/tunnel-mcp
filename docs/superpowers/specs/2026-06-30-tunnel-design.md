...
- **E2E encryption:** the `#<key>` fragment in the join link is a random secret
  generated by the host. `chat` bodies are sealed with libsodium secretbox
  (XSalsa20-Poly1305) using a per-message nonce, before they ever touch the
  wire. Cloudflare sees only ciphertext. Both parties hold the key (the host
  generated it; the guest got it from the link), so both agents can read — the
  *pipe* cannot.
...
- **Confidentiality.** E2E encryption → the pipe sees only ciphertext + metadata
  (timing/size/random hostname). Acceptable; WebRTC is the upgrade path if even
  metadata matters.
