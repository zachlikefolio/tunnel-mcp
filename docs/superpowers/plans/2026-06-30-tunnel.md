...
**Architecture:** One npm package is added to each developer's Claude Code as an MCP server. The initiator ("host") `tunnel_open`s, which turns their MCP process into an in-memory WebSocket relay exposed via a throwaway `cloudflared` Quick Tunnel; the other developer ("guest") `tunnel_join`s by dialing out to that URL. Messages are seq-ordered through the host and sealed with libsodium secretbox so the pipe only sees ciphertext. Teardown (explicit / idle / process-exit) destroys the relay and the session log.
